(function () {
	...
	function addStyles() {
		...
		var styles =
			Lampa.Storage.get("wide_post") !== false
				? getWideStyles()
				: getSmallStyles();
	...
	function preloadAllVisibleCards() {
		...
		preloadTimer = setTimeout(function () {
			...
			shotCards.each(function () {
				...
				var cardImg = $elem.find(".card__img");
				...
				if (title && year) {
					...
					var searchKey =
						"shots_search_" + title + "_" + year + "_" + mediaType;
			...
			for (var i = 0; i < mutations.length; i++) {
				...
				for (var j = 0; j < added.length; j++) {
					...
					if (node.nodeType === 1) {
						...
						if (
							$node.hasClass("card__vote") ||
							$node.hasClass("full-start__rate") ||
							$node.hasClass("info__rate")
						) {
							siStyleApplyColorByRating(node);
						}
						if (
							$node.hasClass("rate--kp") ||
							$node.hasClass("rate--imdb") ||
							$node.hasClass("rate--cub")
						) {
